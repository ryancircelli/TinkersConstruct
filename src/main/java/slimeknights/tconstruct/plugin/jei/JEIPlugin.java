...
public class JEIPlugin implements IModPlugin {
  ...
  public void registerVanillaCategoryExtensions(IVanillaCategoryExtensionRegistration registry) {
    // JEI 19 registers one extension instance per recipe class rather than a factory per recipe; see
    // MaterialsCraftingExtension's own note. A factory that returned null for an unshowable recipe is isHandled now.
    ...
    registry.getCraftingCategory().addExtension(ShapelessMaterialsRecipe.class, ShapelessMaterialsExtension.INSTANCE);
  }
  ...
  public void onRuntimeAvailable(IJeiRuntime jeiRuntime) {
    ...
    if (Config.CLIENT.showPotionFluidInJEI.get()) {
      // 1.21 deleted Potions.EMPTY - a stack simply has no potion contents instead - so water is the only vanilla
      // potion left to filter out by identity here.
      manager.addIngredientsAtRuntime(NeoForgeTypes.FLUID_STACK,
                                      BuiltInRegistries.POTION.holders()
                                        ...
                                        .<FluidStack>map(holder -> PotionFluidType.potionFluid(holder, FluidType.BUCKET_VOLUME)).toList());
    }
    ...
    if (level != null) {
      // JEI 19 hides crafting recipes by holder, not by recipe, matching 1.21's RecipeManager
