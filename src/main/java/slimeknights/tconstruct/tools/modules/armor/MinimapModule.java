...
public enum MinimapModule implements ModifierModule, EquipmentChangeModifierHook, KeybindInteractModifierHook, InventoryTickModifierHook, ModifierRemovalHook, InventorySelectionModule {
      ...
      if (data != null) {
        ...
        if (!map.isEmpty()) {
          ...
          if (holder instanceof ServerPlayer player) {
            ...
            // the map ID is the minecraft:map_id component now, MapItem no longer offers a getter
  ...
  public void onInventorySelect(IToolStackView tool, ModifierEntry modifier, Player player, int newIndex, ItemStack stack) {
    // display the raw number as 1.20 did, the component wraps it in a record
    MapId mapId = stack.get(DataComponents.MAP_ID);
    Integer id = mapId == null ? null : mapId.id();
    player.displayClientMessage(Component.translatable(SELECTED, stack.getHoverName(), id, newIndex + 1), true);
  }
