...
public class InteractionHandler {
    ...
    if (event.getItemStack().isEmpty() && !player.isSpectator()) {
      ...
      if (chestplate.is(TinkerTags.Items.INTERACTABLE_ARMOR) && !player.getCooldowns().isOnCooldown(chestplate.getItem())) {
        ...
        if (useBlock == TriState.TRUE || (useBlock != TriState.FALSE
          ...
          // BlockState#use split in two in 1.21; the empty handed half, which is what a chestplate interact is, is useWithoutItem
      ...
      if (helmet.is(TinkerTags.Items.ARMOR)) {
        ...
        for (ModifierEntry entry : tool.getModifierList()) {
          if (entry.getHook(ModifierHooks.ARMOR_INTERACT).startInteract(tool, entry, player, slotType, modifierKey)) {
            ...
            TinkerDataCapability.getData(player).computeIfAbsent(INTERACT_KEY).put(slotType, new ArmorInteractData(entry.getId(), player.tickCount));
            break;
          }
        }
        return true;
  ...
  public static boolean stopArmorInteract(Player player, EquipmentSlot slotType) {
    if (!player.isSpectator()) {
      ...
      if (helmet.is(TinkerTags.Items.ARMOR)) {
        ...
        // read only, so do not create the data for a player who never started an armor interaction
        TinkerDataCapability.Holder data = TinkerDataCapability.getExistingData(player);
        if (data != null) {
   ...
   * @apiNote  1.21's {@link LeftClickBlock} has no cancellation result: {@code setCancellationResult} lives only on the
   * right click and entity interact events now, as nothing downstream of a left click reads one. The swing and the
   * creative mode cancel, which are what the player actually sees, both survive.
   */
   ...
   * @apiNote  {@code ShieldBlockEvent} became {@link LivingShieldBlockEvent}, which fires whether the entity is
   * blocking or not, where the 1.20 event only fired once vanilla had decided it was. {@link
   * LivingShieldBlockEvent#getOriginalBlock()} carries that decision, so the guard below restores the old firing
   * condition rather than letting the block angle check answer for hits vanilla would never have blocked.
   * {@code setShieldTakesDamage(false)} is a shield damage of zero now, as the event carries an amount instead of a
   * flag.
   */
  @SubscribeEvent
