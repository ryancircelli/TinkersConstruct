...
public class DoubleJumpHandler {
  ...
  /** Clears the jump counter, if the entity ever had one. Only a removal, so never create the data to do it. */
  private static void clearJumps(LivingEntity living) {
    ModDataNBT data = PersistentDataCapability.getExistingData(living);
    if (data != null) {
      data.remove(JUMPS);
    }
  }
   ...
   * @apiNote  Forge's entity gravity attribute became vanilla's {@link Attributes#GRAVITY} in 1.21, same meaning and
   * same 0.08 default, so the negative gravity test is unchanged.
   */
  @SubscribeEvent
