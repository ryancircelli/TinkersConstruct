...
public class ModifierLootModifier extends LootModifier {
  /**
   * Codec for this modifier.
   * @apiNote  {@link IGlobalLootModifier#codec()} returns a {@link MapCodec} in 1.21, and the registry it is
   *           registered to holds map codecs too, so this is {@code RecordCodecBuilder.mapCodec} rather than
   *           {@code create}. The JSON is unchanged.
   */
  ...
  protected ObjectArrayList<ItemStack> doApply(ObjectArrayList<ItemStack> generatedLoot, LootContext context) {
    ...
    if (stack == null) {
      ...
      // 1.21 renamed DIRECT_KILLER_ENTITY to DIRECT_ATTACKING_ENTITY, same parameter under a new name
