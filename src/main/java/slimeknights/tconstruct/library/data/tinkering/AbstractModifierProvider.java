...
public abstract class AbstractModifierProvider extends GenericDataProvider {
  ...
  private record Composable(@Nullable ComposableModifier.Builder builder, @Nullable ICondition condition, JsonRedirect[] redirects) {
    ...
    public JsonObject serialize() {
      ...
      if (condition != null) {
        // ModifierManager reads a single "condition" object with ICondition#CODEC; the registry the codec dispatches
        // through is static, so plain JsonOps is enough and no registry lookup needs threading into this provider
