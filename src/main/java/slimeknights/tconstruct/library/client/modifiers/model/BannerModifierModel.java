...
public record BannerModifierModel(@Nullable ResourceLocation smallPrefix, @Nullable ResourceLocation largePrefix) implements ModifierModel {
  ...
  /**
   * No textures can be validated at bake time.
   * <p>
   * 1.20 walked {@code Sheets.SHIELD_MATERIALS}, a static map of every banner pattern, to ask the sprite getter for
   * each pattern's texture and so log the missing ones. In 1.21 a banner pattern is datapack content in the
   * {@code minecraft:banner_pattern} registry, that map is private and lazily filled from holders nobody has yet, and
   * a datapack registry is not reachable from model baking at all. There is no list to walk, so the missing texture
   * check is the one {@link #addQuads} already makes per pattern actually used.
   */
  @Override
    ...
    if (prefix != null) {
      ...
      if (modData.contains(key, CompoundTag.TAG_LIST)) {
        ...
        for (int i = 0; i < list.size(); i++) {
          // 1.20 stored the pattern as the short hash string vanilla used for banner NBT and mapped it back with
          // BannerPattern#byHash. Both the hash scheme and that method are gone in 1.21 - a pattern is a registry
          // entry now - so the pattern is stored and read as its id, which is what the texture suffix wanted anyway.
