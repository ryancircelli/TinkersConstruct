...
public class ToolPartLootEntry extends LootPoolSingletonContainer {
   ...
   * @apiNote  1.21 replaced the {@code LootPoolSingletonContainer.Serializer} pair with a single {@link MapCodec}
   *           composed on top of {@link LootPoolSingletonContainer#singletonFields}, which reads the shared
   *           {@code weight}, {@code quality}, {@code conditions} and {@code functions}. The JSON is unchanged.
   */
  public static final MapCodec<ToolPartLootEntry> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
    Loadables.ITEM_TAG.codec().fieldOf("tag").forGetter(entry -> entry.tag),
    RandomMaterial.LOADER.codec().fieldOf("material").forGetter(entry -> entry.material)
  ).and(singletonFields(instance)).apply(instance, ToolPartLootEntry::new));
  /** Loot entry type, registered by {@code TinkerToolParts} */
