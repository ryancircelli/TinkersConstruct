  ...
  public float apply(FluidStack fluid, EffectLevel level, FluidEffectContext.Entity context, FluidAction action) {
    ...
    if (player != null) {
      ...
      for (InteractionHand hand : InteractionHand.values()) {
        ...
        // check if a listener wants to override
        PlayerInteractEvent.EntityInteractSpecific event = NeoForge.EVENT_BUS.post(new PlayerInteractEvent.EntityInteractSpecific(player, hand, target, hit));
        InteractionResult result = event.isCanceled() ? event.getCancellationResult() : null;
        // skipped: never spectator mode if we made it this far
        if (result == null) {
          // no override, so find first success from vanilla hooks
