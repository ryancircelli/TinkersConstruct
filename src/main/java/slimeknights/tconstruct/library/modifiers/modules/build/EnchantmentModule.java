...
public interface EnchantmentModule extends ModifierModule, LevelingIntModule, ConditionalModule<IToolStackView> {
   ...
   * @apiNote A {@link Holder}, not a raw {@link Enchantment}: enchantments are a datapack registry in 1.21, so
   * there is no static registry to name one against and the holder Mantle's loader hands back is the only way to
   * carry an identity across a reload. {@link Loadables#ENCHANTMENT} already resolves to this type, so every
   * JSON-configured module gets it for free.
   */
   ...
   * @apiNote Takes a {@link Holder} rather than a {@code ResourceKey}: a datapack registry has no value to hand a
   * Java caller until a registry access is in scope (data generation's {@code HolderLookup.Provider}, or a
   * level's {@code RegistryAccess} at runtime), and resolving one is the caller's problem to solve once, not
   * this builder's to solve on every call.
   */
  ...
  class Constant implements EnchantmentModule, EnchantmentModifierHook {
    ...
    public int updateEnchantmentLevel(IToolStackView tool, ModifierEntry modifier, Holder<Enchantment> enchantment, int level) {
      if (enchantment.is(this.enchantment()) && condition().matches(tool, modifier)) {
  ...
  class Protection extends Constant implements ProtectionModifierHook {
    ...
    /**
     * @apiNote 1.21 moved a per-enchantment protection formula off {@code Enchantment} and onto its effect
     * components: {@code Enchantment#slots} and {@code #getDamageProtection} are gone, replaced by
     * {@code matchingSlot(EquipmentSlot)} and {@code modifyDamageProtection}, which mutates an accumulator
     * rather than returning a value and needs a {@link ServerLevel}. This hook only ever runs as part of the
     * server-authoritative damage sequence (T8a SS3), so a context whose level is not a {@code ServerLevel}
     * (client-side prediction, if any reaches this hook) simply contributes no protection rather than guessing.
     */
    @Override
    public float getProtectionModifier(IToolStackView tool, ModifierEntry modifier, EquipmentContext context, EquipmentSlot slotType, DamageSource source, float modifierValue) {
      if (condition().matches(tool, modifier)) {
        ...
        if (subtractLevel > 0 && enchantment.matchingSlot(slotType) && !source.is(DamageTypeTags.BYPASSES_ENCHANTMENTS) && context.getLevel() instanceof ServerLevel serverLevel) {
  ...
  record MainHandHarvest(Holder<Enchantment> enchantment, LevelingInt level, ModifierCondition<IToolStackView> condition, ResourceLocation conditionFlag, IJsonPredicate<BlockState> block, IJsonPredicate<LivingEntity> holder) implements EnchantmentModule, EnchantmentModifierHook, BlockHarvestModifierHook {
    ...
    @Override
    public int updateEnchantmentLevel(IToolStackView tool, ModifierEntry modifier, Holder<Enchantment> enchantment, int level) {
      if (enchantment.is(this.enchantment()) && tool.getPersistentData().getBoolean(conditionFlag)) {
        level += getLevel(modifier);
      }
      return level;
    }
