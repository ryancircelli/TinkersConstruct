...
public record ArmorLevelModule(TinkerDataKey<Integer> key, boolean allowBroken, @Nullable TagKey<Item> heldTag) implements HookProvider, EquipmentChangeModifierHook, ModifierModule {
  ...
  /**
   * Gets the total level from the key in the entity modifier data
   * @param data   Data holder, may be null if the entity has no data
   * @param key    Key to get
   * @return  Level from the key
   * @apiNote Takes the resolved holder rather than a capability query: {@code LazyOptional} is gone in 1.21
   * (T8a SS2 row 2) and a capability lookup returns the value directly or null, which
   * {@link TinkerDataCapability#getData(LivingEntity)} already does.
   */
