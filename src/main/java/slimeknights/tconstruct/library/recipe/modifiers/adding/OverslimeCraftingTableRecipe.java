...
public class OverslimeCraftingTableRecipe extends CustomRecipe {
  ...
  public ItemStack assemble(CraftingInput inv, HolderLookup.Provider registryAccess) {
    ToolFound match = findTool(inv, tools, ingredient);
    if (match == null) {
      // a recipe cannot name itself in 1.21; the ingredient it repairs with is the most identifying thing reachable
      TConstruct.LOG.error("Overslime crafting table recipe using {} failed to find tool after matching", ingredient);
