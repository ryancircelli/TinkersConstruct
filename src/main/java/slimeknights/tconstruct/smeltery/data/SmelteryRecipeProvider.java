...
public class SmelteryRecipeProvider extends BaseRecipeProvider implements ISmelteryRecipeHelper, ICommonRecipeHelper {
  ...
  /** Builds a water potion stack; PotionUtils is gone and potion contents are a data component now (M7 §9) */
  private static ItemStack potion(Item item) {
    return PotionContents.createItemStack(item, Potions.WATER);
  }
  ...
  private void addSmelteryRecipes(RecipeOutput consumer) {
    ...
    // 1.20 wrote a forge:conditional file whose first branch re-serialized this recipe under the ceramics:kiln
    // type when Ceramics was loaded. Neither half survives: forge:conditional is gone (conditions live on an ordinary
    // recipe file now), and the serializer override Mantle's ConsumerWrapperBuilder provided has no counterpart at
    // all, because a recipe's type comes from the recipe object rather than from whoever writes it. So only the
    // blasting recipe is written, unconditionally, and the kiln variant is compat work for the slice that brings
    // Ceramics back (T20) to write as its own recipe.
    fastGrout.accept(consumer);
  ...
  private void addCastingRecipes(RecipeOutput consumer) {
    ...
    ItemCastingRecipeBuilder.tableRecipe(ItemOutput.fromStack(potion(Items.POTION)))
                            .setFluid(MantleTags.Fluids.WATER, FluidValues.BOTTLE * 2)
                            .setCoolingTime(1)
                            .setCast(Items.GLASS_BOTTLE, true)
                            .save(consumer, location(waterFolder + "bottle"));
    ItemCastingRecipeBuilder.tableRecipe(ItemOutput.fromStack(potion(Items.SPLASH_POTION)))
                            .setFluid(MantleTags.Fluids.WATER, FluidValues.BOTTLE * 2)
                            .setCoolingTime(1)
                            .setCast(MantleTags.Items.SPLASH_BOTTLE, true)
                            .save(consumer, location(waterFolder + "splash"));
    ItemCastingRecipeBuilder.tableRecipe(ItemOutput.fromStack(potion(Items.LINGERING_POTION)))
                            .setFluid(MantleTags.Fluids.WATER, FluidValues.BOTTLE * 2)
                            .setCoolingTime(1)
  ...
  private void addMeltingRecipes(RecipeOutput consumer) {
    ...
    // NeoForge dropped forge:glass_silica outright, with no c: successor (unlike glass/glass_colorless above); the closest
    // surviving tag is the general glass_blocks one used above, which is a proper superset. Tinkers has the analogous
    // narrower TinkerTags.Items.GLASS_PANES_SILICA for panes below but nothing yet for blocks - a genuine gap, not a rename,
    // flagged as a finding rather than inventing a new tag + datapack population here.
  ...
  private void addAlloyRecipes(RecipeOutput consumer) {
    ...
    // "first passing condition wins" is rebuilt the way SmelteryRecipeBuilder does it: each alternative is its own
    // file, conditioned on its own condition and the negation of every earlier one, and the fallback keeps the plain id
    ...
    RecipeOutput pewterOutput = withCondition(consumer, new OrCondition(List.of(ConfigEnabledCondition.ALLOW_INGOTLESS_ALLOYS, tagCondition("ingots/pewter"))));
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenPewter, FluidValues.INGOT * 4)
      ...
      .save(pewterOutput.withConditions(new AndCondition(List.of(lead, tin))), pewterId.withSuffix("_tin_lead"));
    // otherwise, substitute iron for the missing part
    // metalborn does pewter without lead
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenPewter, FluidValues.INGOT * 4)
      .addInput(TinkerFluids.moltenTin.ingredient(FluidValues.INGOT * 3))
      .addInput(TinkerFluids.moltenIron.ingredient(FluidValues.INGOT))
      .save(pewterOutput.withConditions(new NotCondition(new AndCondition(List.of(lead, tin))), tin), pewterId.withSuffix("_tin"));
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenPewter, FluidValues.INGOT * 2)
      .addInput(TinkerFluids.moltenIron.ingredient(FluidValues.INGOT))
      .addInput(TinkerFluids.moltenLead.ingredient(FluidValues.INGOT))
      .save(pewterOutput.withConditions(new NotCondition(new AndCondition(List.of(lead, tin))), new NotCondition(tin), lead), pewterId);
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenNicrosil, FluidValues.INGOT * 4)
      ...
      .addInput(TinkerFluids.moltenQuartz.ingredient(FluidValues.GEM))
      .save(withCondition(consumer, chromium, nickel), nicrosilId.withSuffix("_chromium_nickel"));
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenNicrosil, FluidValues.INGOT * 4)
      ...
      .addInput(TinkerFluids.moltenQuartz.ingredient(FluidValues.GEM))
      .save(withCondition(consumer, new NotCondition(chromium), nickel), nicrosilId.withSuffix("_nickel"));
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenNicrosil, FluidValues.INGOT * 4)
      ...
      .addInput(TinkerFluids.moltenIron.ingredient(FluidValues.INGOT))
      .addInput(TinkerFluids.moltenQuartz.ingredient(FluidValues.GEM))
      .save(withCondition(consumer, new NotCondition(nickel), chromium), nicrosilId.withSuffix("_chromium"));
    ...
    AlloyRecipeBuilder.alloy(TinkerFluids.moltenNicrosil, FluidValues.INGOT * 4)
      ...
      .addInput(TinkerFluids.moltenEmerald.ingredient(FluidValues.GEM))
      .addInput(TinkerFluids.moltenQuartz.ingredient(FluidValues.GEM))
      .save(withCondition(consumer, new NotCondition(nickel), new NotCondition(chromium), tagCondition("ingots/tin")), nicrosilId);
