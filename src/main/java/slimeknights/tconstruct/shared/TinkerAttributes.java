...
public class TinkerAttributes {
   ...
   *             1.21 has no ambient currently-loading-mod context to fetch it from.
   */
  public TinkerAttributes(IEventBus bus) {
   ...
   * @apiNote  1.21 added a vanilla {@link Attributes#SAFE_FALL_DISTANCE} that means the same thing and is read by
   *           {@code LivingEntity#calculateFallDamage}. Ours is deliberately kept for now: Tinkers subtracts this
   *           value in its own fall damage handler, so folding onto vanilla's without also deleting that subtraction
   *           would reduce fall damage twice. The fold belongs with whoever ports {@code tools/logic}.
   */
