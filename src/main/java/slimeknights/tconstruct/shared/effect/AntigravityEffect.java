...
public class AntigravityEffect extends TinkerEffect {
  ...
  /** Cached holder for this effect, as 1.21 looks effects up on entities by holder rather than by instance */
  @Nullable
  private Holder<MobEffect> holder;

  public AntigravityEffect() {
    super(MobEffectCategory.HARMFUL, 0xff970d, true);
    // 1.21 moved gravity from a Forge extension attribute into vanilla as Attributes.GRAVITY
    ...
    // 1.21 removed the entity and amplifier from MobEffect#removeAttributeModifiers, so the velocity cache is cleared
    // from the effect removal events instead
    NeoForge.EVENT_BUS.addListener(this::onEffectRemoved);
    NeoForge.EVENT_BUS.addListener(this::onEffectExpired);
  }

  /** Gets the registry holder for this effect */
  private Holder<MobEffect> holder() {
    if (holder == null) {
  ...
  /** Drops the cached velocity for an entity that just lost this effect */
  private void clearVelocity(LivingEntity living, Holder<MobEffect> effect) {
    if (effect.value() == this) {
      LAST_VELOCITY.remove(living.getId());
    }
  }

  /** Handles the effect being cured or otherwise removed early */
  private void onEffectRemoved(MobEffectEvent.Remove event) {
    clearVelocity(event.getEntity(), event.getEffect());
  }

  /** Handles the effect running out of duration */
  private void onEffectExpired(MobEffectEvent.Expired event) {
    clearVelocity(event.getEntity(), event.getEffectInstance().getEffect());
  }
