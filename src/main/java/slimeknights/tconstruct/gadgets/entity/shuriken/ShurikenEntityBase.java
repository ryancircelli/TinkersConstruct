 ...
 * @implNote  1.20's {@code writeSpawnData}/{@code readSpawnData}/{@code getAddEntityPacket} overrides synced the
 *            held item over a custom spawn packet ({@code IEntityAdditionalSpawnData}, {@code NetworkHooks}) -
 *            neither exists in 1.21. Both are gone unreplaced: {@link ThrowableItemProjectile} already syncs its
 *            item through {@code SynchedEntityData}, which the default {@link Entity#getAddEntityPacket()} already
 *            delivers on spawn, so there was nothing left for the override to add.
 */
@Deprecated
