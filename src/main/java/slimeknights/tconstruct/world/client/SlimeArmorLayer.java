  ...
  public void render(PoseStack matrices, MultiBufferSource buffer, int packedLight, T entity, float pLimbSwing, float swing, float partialTicks, float age, float headYaw, float headPitch) {
    ...
    if (!helmet.isEmpty()) {
      ...
      if (item instanceof ArmorItem armor && armor.getType() == ArmorItem.Type.HELMET) {
        ...
        // 1.21 moved off a single texture plus a dyeable check onto a list of layers, each with its own tint -
        // see HumanoidArmorLayer#renderArmorPiece, which this mirrors for the single head slot.
        ...
        for (int layerIdx = 0; layerIdx < layers.size(); layerIdx++) {
          ...
          int color = extensions.getArmorLayerTintColor(helmet, entity, layer, layerIdx, fallbackColor);
          if (color != 0) {
