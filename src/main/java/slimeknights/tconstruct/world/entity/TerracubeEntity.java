    ...
    if (this.random.nextFloat() < 0.15F * multiplier) {
      ...
      if (current.isEmpty()) {
        ...
        if (item != null) {
          ...
          // enchantSpawnedArmor gained a level parameter and reordered slot before difficulty; this always runs
          // server-side (spawn finalization), so the cast off this.level() is safe
